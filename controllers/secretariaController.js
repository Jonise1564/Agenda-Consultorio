...
//     // 1. Panel Principal de Secretaría
//     async index(req, res, next) {
//         try {
//             const especialidades = await Especialidad.getAll();
//             const medicos = await Medico.listar();
...
//     // 3. Consulta de slots horarios disponibles (AJAX)
//     async disponibilidad(req, res, next) {
//         try {
//             const { id_medico, id_especialidad, fecha } = req.query;
...
//                 const horaStr = actual.toTimeString().slice(0, 5);
//                 if (ocupados.includes(horaStr)) {
//                     html += `<div class="slot-hora ocupado" style="opacity:0.5; cursor:not-allowed; background:#e9ecef; width: 85px; border: 2px solid #dee2e6; border-radius: 8px; padding: 12px 5px; text-align: center; margin: 2px;"><span>${horaStr}</span></div>`;
//                 } else {
//                     html += `<div class="slot-hora" data-hora="${horaStr}" data-agenda="${agenda.id}" style="width: 85px; border: 2px solid #dee2e6; border-radius: 8px; padding: 12px 5px; text-align: center; cursor: pointer; background: white; font-weight: bold; margin: 2px;"><span>${horaStr}</span></div>`;
//                 }
//                 actual.setMinutes(actual.getMinutes() + agenda.duracion_turnos);
//             }
//             res.send(html || '<p class="text-center">No hay intervalos disponibles.</p>');
...
//     // 4. LISTADO DE TURNOS (Este es el que te faltaba)
//     async verListaTurnos(req, res, next) {
//         try {
//             const { paciente, profesional, fecha, status } = req.query;
//             const turnos = await Turno.listarConFiltros({
...
//     // 3. Consulta de disponibilidad (RETORNA JSON - CONTROLADOR LIMPIO)
//     async disponibilidad(req, res, next) {
//         try {
//             const { id_medico, id_especialidad, fecha } = req.query;
...
//             // B) Verificar Ausencia del médico
//             const ausencia = await Agenda.obtenerAusencia(id_medico, fecha);
//             if (ausencia) {
//                 return res.json({ 
//                     status: 'ausencia', 
...
//             // D) Generar Array de Horarios (Dato puro, no HTML)
//             let horarios = [];
//             let [h, m] = agenda.hora_inicio.split(':');
//             let actual = new Date(2000, 0, 1, h, m);
//             let [hFin, mFin] = agenda.hora_fin.split(':');
...
class SecretariaController {
    ...
    // 3. Consulta de slots horarios (RETORNA JSON)
    async disponibilidad(req, res, next) {
        try {
            const { id_medico, id_especialidad, fecha } = req.query;
    ...
    // 4. Listado de Turnos con Filtros
    async verListaTurnos(req, res, next) {
        try {
            const { paciente, profesional, fecha, status } = req.query;
            const turnos = await Turno.listarConFiltros({
    ...
    // 5. Proceso de agendar (CON VALIDACIÓN DE FECHA)
    async agendar(req, res, next) {
        try {
            const { id_paciente, motivo, fecha, hora_inicio, id_agenda } = req.body;
            const archivo_dni = req.file ? req.file.filename : null;
