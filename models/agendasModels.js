...
class Agenda {
    ...
    static async getAll() {
        ...
        try {
            ...
            SELECT 
                a.id, a.id_medico, a.fecha_creacion, a.fecha_fin,
                p.nombre AS nombre, p.apellido AS apellido,
                GROUP_CONCAT(DISTINCT d.dia ORDER BY d.id SEPARATOR ', ') AS dias, -- <--- CORREGIDO: "ORDER BY"
                a.hora_inicio, a.hora_fin, a.limite_sobreturnos, a.duracion_turnos,
                e.nombre AS especialidad, s.nombre AS sucursal, c.nombre AS clasificacion
            FROM agendas a
            ...
            WHERE m.estado = 1 AND a.activo = 1
            GROUP BY a.id, a.id_medico, a.fecha_creacion, a.fecha_fin, p.nombre, p.apellido,
                     a.hora_inicio, a.hora_fin, a.limite_sobreturnos, a.duracion_turnos,
    ...
    static async getAgendaById(id) {
        ...
        try {
            ...
            WHERE a.id = ? AND a.activo = 1; -- <--- FILTRO AGREGADO
        `, [id]);
    ...
    // =====================================================
    // 5. DESACTIVAR AGENDA (BORRADO LÓGICO)
    // =====================================================
    static async eliminar(id) {
        let conn;
        try {
            conn = await createConnection();
            // Solo actualizamos el campo activo a 0
            const [result] = await conn.query(`UPDATE agendas SET activo = 0 WHERE id = ?`, [id]);
            return result.affectedRows > 0;
        } catch (error) {
            console.error('Error en desactivar agenda:', error);
            throw error;
        } finally {
            if (conn) conn.end();
        }
    ...
    static async existeSolapamiento(data) {
        ...
        try {
            ...
            WHERE a.id_medico = ? 
              ...
              AND a.activo = 1  -- <--- FILTRO AGREGADO: Ignorar agendas desactivadas
    ...
    static async existeSolapamientoUpdate(datos = {}) {
        ...
        try {
            ...
            WHERE a.id_medico = ? 
              AND a.id_especialidad = ?
              AND a.id != ?      -- Ignora la agenda actual que se está editando
              AND a.activo = 1   -- <--- FILTRO AGREGADO: Ignora agendas desactivadas
              AND (a.fecha_creacion <= ? AND a.fecha_fin >= ?)
